...
class Visualizer:
    ...
    def plot_signals(self, strategy):
        ...
        try:
            ax.plot(
                self._data.index, self._data["Close"], label="Close Price", linewidth=2
            )
    ...
    def plot_signals_ml(self, ml_df):
        ...
        try:
            ax.plot(ml_df.index, ml_df["Close"], label="Close Price", linewidth=2)
            signal_col = "ML_Signal"
            buy_signals = ml_df[ml_df[signal_col] == 1]
            ax.scatter(
                buy_signals.index,
                buy_signals["Close"],
                marker="^",
            ...
            sell_signals = ml_df[self.data[signal_col] == -1]
            ax.scatter(
                sell_signals.index,
                sell_signals["Close"],
                marker="^",
